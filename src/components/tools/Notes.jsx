...
const Notes = () => {
  ...
  const extractTitle = (content) => {
    ...
    const cleanTitle = firstLine.replace(/^#+\s*/, '').replace(/\*\*(.*?)\*\*/g, '$1').replace(/\*(.*?)\*/g, '$1')
  ...
  const insertMarkdown = (format) => {
    ...
    const newContent = currentNote.substring(0, start) + newText + currentNote.substring(end)
  ...
  const renderMarkdownPreview = (content) => {
    return content
      ...
      .replace(/`(.*?)`/g, '<code class="bg-gray-800 px-2 py-1 rounded text-accent-1 font-mono text-sm">$1</code>')
      .replace(/^- (.*$)/gm, '<li class="text-gray-300 ml-4 mb-1">• $1</li>')
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-accent-1 hover:text-accent-2 underline" target="_blank">$1</a>')
      .replace(/\n/g, '<br>')
  }
        ...
        <div className="flex-1 flex flex-col">
          ...
          {previewMode ? (
            <div 
              ...
              dangerouslySetInnerHTML={{ __html: renderMarkdownPreview(currentNote) }}
            />
          ) : (
            <textarea
              ref={textareaRef}
              ...
              placeholder={selectedNoteId ? "Start writing your note... Use markdown for formatting!" : "Create a new note to get started"}
          ...
          <div className="mt-4 text-xs text-gray-500">
            <details className="cursor-pointer">
              <summary className="hover:text-gray-400 transition-colors">Markdown formatting help</summary>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-3">
                    ...
                    />
                    <span className="text-sm text-gray-400">Auto-save notes</span>
                  </label>
                  ...
                  <label className="flex items-center gap-3">
                    <input
                      ...
                      className="w-4 h-4 text-accent-1 bg-gray-700 border-gray-600 rounded focus:ring-accent-1 focus:ring-2"
                    />
                    <span className="text-sm text-gray-400">Show formatting toolbar</span>
                  </label>
                </div>
              </div>
            </div>
