...
const chromeExtDir = path.join(projectRoot, 'extensions', 'chrome');
...
try {
  // Create build directory for Chrome extension
  const chromeBuildDir = path.join(chromeExtDir, 'build');
  ...
  if (fs.existsSync(distDir)) {
    copyDir(distDir, chromeBuildDir);
  } else {
  ...
  for (const file of extensionFiles) {
    const srcPath = path.join(chromeExtDir, file);
    const destPath = path.join(chromeBuildDir, file);
  ...
  const buildIconsDir = path.join(chromeBuildDir, 'icons');
...
<body>
  ...
  fs.writeFileSync(path.join(chromeBuildDir, 'welcome.html'), welcomeHtml);
  
  console.log('✅ Chrome extension built successfully!');
  console.log(`📁 Extension files are in: ${chromeBuildDir}`);
